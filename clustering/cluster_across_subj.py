...
def extract_peaks(dataset=BI2013a(), db_name = "BI2013a"):
    ...
    for subj in dataset.subject_list:
        ...
        #save
        path = os.path.join(os.path.dirname(__file__), '../peak_data/{}'.format(db_name))
...
def extract_window_mean(dataset=BI2013a(), db_name = "BI2013a"):
    ...
    XY = {"ERP":[], "Target":[], "Timeidx":[], "Session":[], "Subject":[]}
    ...
    for subj in dataset.subject_list:
        ...
        for session in unique_sessions:
            ...
            #TODO add epoch id using metadata
...
def read_pkl_files(folder = '../peak_data', db_name = '*'):
    """
    ...
    pkls_all_subj = [d for d in glob.glob(path + "{}/*".format(db_name))]
    print(pkls_all_subj)
    dict_subj = {}
    ...
    #for all subjects    
    df_peaks = pd.DataFrame(dict_subj)
    return df_peaks
...
for subj in dataset.subject_list[3:]:
    ...
    #save
    path = os.path.join(os.path.dirname(__file__), '../window_mean_data/{}/'.format(db_name))
    #verify if folder exists, create otherwise
    os.makedirs(path, exist_ok=True)
    # Filepath for the image
    dict_path = os.path.join(path, 'XY_{}_subj{}.pkl'.format(db_name,subj))
